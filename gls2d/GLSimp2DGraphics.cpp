...
s2d::S2DGraphics::S2DGraphics(UINT32 width, UINT32 height) {
	...
	const char* vertexShaderCode =
		...
		"layout (location = 2) in vec2 texPos;\n"
		"layout (location = 3) in float texID;\n"
		"layout (location = 4) in float fRotAngle;\n"
		"out vec4 oCol;\n"
		"out vec2 texCoord;\n"
		"out float isTexured; \n"
		"out float fTexID;\n"
		"uniform float screenParam[2];\n"

		"vec2 normalizePoint(vec2 point) { \n"
		"	return vec2( ( point.x / screenParam[0] * 2.0) - 1.0,(point.y / screenParam[1] * 2.0) - 1.0); \n"
		"}\n"
	...
	s2d::INT32 windowParamLoc = glGetUniformLocation(m_glShaderProgID, "screenParam");
	glDeleteShader(px);
	glDeleteShader(vx);
	int temp[32] = { 0,1 };
	...
	float winParam[] = { width, height };
	glUniform1fv(windowParamLoc, 2, winParam);
...
void s2d::S2DGraphics::drawRect(F32 x, F32 y, F32 width, F32 height, Color color, Texture texture)
{
	...
	Vec2f fP = /*normalizePoint({x, y}); */{x, y};
	Vec2f fOff = /*normalizePoint({x + width, y - height});*/{x + width, y - height};
...
void s2d::S2DGraphics::drawCircle(Point p1, float fRadius, Color color, uint32_t nSegmentCount) {
	...
	for (uint32_t i = 0; i < nSegmentCount; i+=2) {
		...
		m_vertices[index + t] = {	p1,		fR, fG, fB, fA, { -1.0f, -1.0f }, -1.0f };
		m_vertices[index + t + 1] = { p2,	fR, fG, fB, fA, { -1.0f, -1.0f }, -1.0f };
		m_vertices[index + t + 2] = { p3,	fR, fG, fB, fA, { -1.0f, -1.0f }, -1.0f };
		m_vertices[index + t + 3] = { p4,	fR, fG, fB, fA, {-1.0f, -1.0f}, -1.0f };
		t += 4;
	}
...
void s2d::S2DGraphics::resetTexture(unsigned char* pixels, int width, int height, Texture* text) {
	int index = text->texID;
	glBindTexture(GL_TEXTURE_2D, m_textureSlot.texID[index]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
...
void s2d::S2DGraphics::getScreenParam(uint32_t* w, uint32_t* h) {
	
	if (w == nullptr || h == nullptr)return;
