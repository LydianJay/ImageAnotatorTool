...
namespace s2d {
	...
	s2d::KeyState S2DWindow::PeekKeyState() {
		return m_keyState;
	}

	s2d::MouseState S2DWindow::PeekMouseState() {
		return m_mouseState;
	}

	void S2DWindow::clearKeyState() {
		m_keyState = { 0, S2D_KEY_NO_INPUT };
	}

	void S2DWindow::clearMouseState() {
		m_mouseState = { S2D_MOUSE_IDLE, S2D_MOUSE_IDLE, {-1.0f, -1.0f} };
	}
	...
	void S2DWindow::openFileExplorer(char* buffer, size_t max_size)
	{
		OPENFILENAME File = { 0 };
		File.hwndOwner = m_hWinhwn;
		File.lpstrFile = buffer;
		memset(File.lpstrFile, '\0', max_size);
		File.lStructSize = sizeof(OPENFILENAME);
		File.nMaxFile = max_size;
	
		GetOpenFileName(&File);
	}

	bool S2DWindow::saveFileExplorer(char* buffer, size_t size) {
		...
		bool res = GetSaveFileNameA(&File);
		return res;
	}
